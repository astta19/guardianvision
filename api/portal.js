...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    return res.status(502).json({ erro: 'Não foi possível conectar ao banco de dados. Tente novamente.' });
  }
  ...
  try {
    ...
    console.error('portal json parse error:', err);
    return res.status(500).json({ erro: 'Resposta inválida do banco de dados' });
  }
