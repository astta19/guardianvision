...
async function toggleDocGenMenu() {
  ...
  if (menu) menu.style.display = menu.style.display === 'none' ? 'block' : 'none';
    ...
    async function showLearningStats() {
      ...
      const contentEl = document.getElementById('learningStatsContent') || document.querySelector('#learningStatsModal .m-box');

      try {
        const svc = getLearningService();
        const [ragResult, docsResult, countResult] = await Promise.all([
          sb.from('interacoes_chat').select('id', { count: 'exact', head: true }).eq('user_id', currentUser.id).gte('feedback_usuario', 4).catch(() => ({ count: 0 })),
          sb.from('documentos_fiscais').select('id', { count: 'exact', head: true }).eq('user_id', currentUser.id).catch(() => ({ count: 0 })),
          supabaseProxy('buscar_treinamento_count', {}).catch(() => ({ count: 0 }))
        ]);

        const countRAG = ragResult?.count || 0;
        const countDocs = docsResult?.count || 0;
        const countTreinamento = countResult?.count || 0;

        // Buscar últimas interações para métricas
        const { data: interacoes } = await sb
          .from('interacoes_chat')
          .select('feedback_usuario, tokens_usados, modelo_usado, criado_em')
          .eq('user_id', currentUser.id)
          .order('criado_em', { ascending: false })
          .limit(50)
          .catch(() => ({ data: [] }));
        ...
        console.error('showLearningStats error:', e);
        const statsDiv = document.getElementById('learningStatsContent');
